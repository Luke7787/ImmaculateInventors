...
describe('item services', () => {
  ...
  describe('getItemsFromUser', () => {
    it('returns items for a given user', async () => {
      ...
      await ItemModel.create({ userId: 'dawglogsondog', name: 'Foot', quantity: 3, folder : mongoose.Types.ObjectId()});
      await ItemModel.create({ userId: 'abcdeffasdf', name: 'Foot', quantity: 5, folder : mongoose.Types.ObjectId() });
      await ItemModel.create({ userId: 'abcdeffasdfasdf', name: 'Foot', quantity: 5, folder : mongoose.Types.ObjectId() });
  ...
  describe('findItemByName', () => {
    it('finds the item by name', async () => {
		const ItemModel = conn.model('Item', ItemSchema, 'items');
		await ItemModel.create({ userId: 'dawglogsondog', name: 'Toe', quantity: 3 , folder : mongoose.Types.ObjectId()});
  ...
  describe('addItem', () => {
    it('adds items', async () => {
      ...
      const itemToAdd = { userId: 'dawglogsondog', name: 'Foot', quantity: 3, folder : folder };
  ...
  describe('deleteItem', () => {
    it('deletes items', async () => {
      ...
      const savedFolder = await newFolder.save();
      const newItem = await ItemModel.create({ userId: savedUser._id.toString(), name: 'Toe', quantity: 3 , folder : folder});
  ...
  describe('findItemById', () => {
    it('all items', async () => {
      ...
      const ItemModel = conn.model('Item', ItemSchema, 'items');
      const newItem = await ItemModel.create({ userId: savedUser._id.toString(), name: 'Toe', quantity: 3 , folder : mongoose.Types.ObjectId()});
